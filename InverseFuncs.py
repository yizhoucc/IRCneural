...
def theta_range(theta, gains_range, std_range, goal_radius_range, Pro_Noise=None, Obs_Noise=None):
    ...
    if type(theta) == tuple:
        ...
        theta[0][1].data.clamp_(
            gains_range[2], gains_range[3])  # [proc_gain_ang]

        if Pro_Noise is None:
            theta[1][0].data.clamp_(
                std_range[0], std_range[1])  # [proc_vel_noise]
            theta[1][1].data.clamp_(
                std_range[2], std_range[3])  # [proc_ang_noise]
        else:
            theta[2:4].data.copy_(Pro_Noise.data)

        theta[2][0].data.clamp_(
            gains_range[0], gains_range[1])  # [obs_gain_vel]
        theta[2][1].data.clamp_(
            gains_range[2], gains_range[3])  # [obs_gain_ang]

        if Obs_Noise is None:
            theta[3][0].data.clamp_(
                std_range[0], std_range[1])  # [obs_vel_noise]
            theta[3][1].data.clamp_(
                std_range[2], std_range[3])  # [obs_ang_noise]
        else:
        ...
        if Pro_Noise is None:
            theta[2].data.clamp_(std_range[0], std_range[1]
                                 )  # [proc_vel_noise]
            theta[3].data.clamp_(std_range[2], std_range[3]
                                 )  # [proc_ang_noise]
        else:
            theta[2:4].data.copy_(Pro_Noise.data)
    ...
    def _wrapped_call(ep, task):
        ...
        for sample_index in range(samples):
            ...
            if mkactionep == [] or mkactionep.shape[0] <= 3:
                continue
            env.reset(theta=theta, phi=phi, goal_position=task,
                      vctrl=states[ep][0][3]/env.phi[0], wctrl=states[ep][0][4]/env.phi[1])
            numtime = len(mkactionep)

            # compare mk data and agent actions
            # use a t and s t (treat st as st+1)
            for t, mk_action in enumerate(mkactionep):
                # agent's action
                action = agent(env.decision_info)
                # agent's obs, last step obs doesnt matter.
                if t < len(states[ep])-1:
...
def monkeyloss(agent=None,
    ...
    def _wrapped_call(ep, task):
        ...
        for sample_index in range(samples):
            ...
            env.reset(theta=theta, phi=phi, goal_position=task,
                      vctrl=mkactionep[0][0], wctrl=mkactionep[0][1])
            logllsample = torch.zeros(
                1).cuda()[0] if gpu else torch.zeros(1)[0]
            agentstop = False
            while True:
                action = torch.zeros(2) if agentstop else agent(
                    env.decision_info)
                agentstop = torch.norm(action) < env.terminal_vel
                ...
                if not mkstop:
                    if type(states[ep]) == list:
                        ...
                        nextstate = states[ep][1:][int(
                            env.trial_timer)].view(-1, 1)
    ...
    if eaction is None:  # just get expert trajectory
        with torch.no_grad():
            while episode < NUM_EP:
                ...
                obs_ep = []
                belief_ep = []
                task_ep = [[env.goalx, env.goaly],
                           env.phi] if not is1d else [env.goalx, env.phi]
                done = False
                while not done:
                ...
                if estate is not None:
                    if type(estate[episode]) == list:
                        if t+1 < len(estate[episode]):
                            _, done = env(
                                eaction[episode][t], task_param=theta, state=estate[episode][t+1])
                        else:
                            pass
                        ...
                        if t+1 < (estate[episode].shape[1]):
                            _, done = env(
                                eaction[episode][t], task_param=theta, state=estate[episode][:, t+1].view(-1, 1))
                else:
...
def trajectory_(agent, theta, env, arg, gains_range, std_range, goal_radius_range, NUM_EP):
    ...
    '''
    pro_gains, pro_noise_stds, obs_gains, obs_noise_stds,  goal_radius = torch.split(
        theta.view(-1), 2)
    ...
    while episode <= NUM_EP:  # defalt 500
        ...
        while t < arg.EPISODE_LEN:  # for a single FF
            ...
            # env(action)
            # so env.x env.belief updated by action and next o
            env(action.view(-1))
            # the same next obs used to update belief
            obs_traj_ep.append(env.o)

            # check time limit
            # if the monkey can't catch the firefly in EPISODE_LEN, reset the game.
            TimeEnd = (t+1 == arg.EPISODE_LEN)
            # mask = 0: episode is over
            mask = torch.tensor([1 - float(TimeEnd)])
            ...
            for t, teacher_action in enumerate(a_traj_ep):
                ...
                if states is None:
                    ...
                    if t+1 < len(states[ep]):
                        decision_info, _ = env(
                            teacher_action, task_param=theta, state=states[ep][t+1])
                action_loss = mse_loss(action, teacher_action)
                obs_loss = mse_loss(env.observations(
                    states[ep][t]), env.observations_mean(states[ep][t]))
                logPr_ep = logPr_ep + action_loss.sum() + obs_loss.sum()
                # return logPr_ep
                ...
                if states is None:
                    ...
                    if type(states[ep]) == list:
                        ...
                        if t+1 < (states[ep]).shape[1]:
                            decision_info, _ = env(
                                teacher_action, task_param=theta, state=states[ep][:, t+1].view(-1, 1))

                action_loss = logll(torch.tensor(
                    teacher_action), action, std=np.sqrt(action_var))
                obs_loss = logll(env.o, env.observations_mean(
                    env.s), std=theta.clone().detach()[4:6])
                logPr_ep = logPr_ep + action_loss.sum() + obs_loss.sum()
                action_loss.detach()
                obs_loss.detach()
...
def logllv1(true=None, estimate=None, std=0.3, error=None):
    ...
    if error is not None:
        loss = torch.log(torch.sqrt(
            torch.tensor([2*pi]))*std)+1/2*(error/std)**2
    else:
        loss = torch.log(torch.sqrt(torch.tensor(
            [2*pi]))*std)+1/2*((estimate-true)/std)**2
    return loss
...
def getLoss_(agent, x_traj, obs_traj, a_traj, theta, env, gains_range, std_range, PI_STD, NUM_SAMPLES):
    ...
    # what is the numsample/particle here? default is 50
    for num_it in range(NUM_SAMPLES):
        ...
        for ep, x_traj_ep in enumerate(x_traj):  # repeat for 501 episodes
            ...
            for it, next_x in enumerate(x_traj_ep[1:]):
                ...
                # multiplied by observation gain, no noise
                next_ox = env.observations_mean(next_x)
                # next_ox_ = env.observations(next_x)  # simulated observation (with noise)

                action_loss = 5*torch.ones(2)+np.log(np.sqrt(2 * pi)*PI_STD) + (
                    action - a_traj_ep[it]) ** 2 / 2 / (PI_STD**2)
                ...
                next_b, info = env.belief_step(
                    env.b, next_ox, a_traj_ep[it], env.box)  # no change to internal var
                env.b = next_b
...
def init_theta(phi, arg, purt=None):
    ...
    theta = theta_range(theta, arg.gains_range, arg.std_range,
                        arg.goal_radius_range)  # keep inside of trained range
    return theta
